...
export const getUserBalances = query({
    handler:async(ctx)=>{
      try {
        // Guard: return empty balances when unauthenticated instead of throwing
        ...
        // Resolve user record directly from the identity to avoid calling
        // internal.users.getCurrentUser (which may run in a different context
        // without auth and throw). This mirrors the lookup used in users.store.
        ...
        const balanceByUser = {}; // { otherUserId: balanceAmount }
        ...
        return {
          youOwe,
          youAreOwed,
          totalBalance: youAreOwed - youOwe > 0 ? youAreOwed - youOwe : 0,
          oweDetails: { youOwe: youOweList, youAreOwedBy: youAreOwedByList },
        };
      } catch (err) {
        // Log server-side and return safe default so client doesn't receive a server error
...
export const getMonthlySpending = query({
  handler: async (ctx) => {
    try {
      const identity = await ctx.auth.getUserIdentity();
      if (!identity) {
        // return zeroed months for the current year
        const currentYear = new Date().getFullYear();
        const months = [];
        for (let i = 0; i < 12; i++) {
          const monthDate = new Date(currentYear, i, 1).getTime();
          months.push({ month: monthDate, total: 0 });
        }
        return months;
      }
      ...
      groups.map(async (group) => {
        ...
        settlements.forEach((settlement) => {
          if (settlement.paidByUserId === user._id) {
            // User paid someone (payer: balance goes down)
            balance -= settlement.amount;
          } else {
            // Someone paid the user (receiver: balance goes up)
            balance += settlement.amount;
          }
        });
